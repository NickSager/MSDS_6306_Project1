...
beers_breweries %>% select(Style,IBU) %>% 
  ...
  print(n = 10)
```
There are still 52 missing IBU values after imputing the mean IBU value for the style of beer. These beers are styles that do not commonly have IBU measured, and so there is no mean to impute. As shown in the table of remaining missing values per style, these beers are largely styles that are irrelevant to our analysis. We will leave the values as missing and they will be excluded in any further analysis.
