...
There are few enoug missing ABV values to be filled in manually using the following code. Values were obtained from websites like BeerAdvocate and Untappd.
```{r}
...
The remaining missing values will be imputed using the mean ABV and IBU values for the style of beer. American IPA's have the highest number of missing IBU values but, as we will see later, the have a unique range of IBU values, so the imputation should be accurate. The same applies to the other Ales that make up the bulk of the missing data. Because our analysis is concerned with Ales, more effort could be put into finding these missing values if more time or money becomes available and a more accurate analysis is desired.
```{r}
...
## NEED TO ADD IBU with facet wrap somehow. Will work on later
beers_medians %>%
